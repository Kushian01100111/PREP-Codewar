...
//2 arrays => Only numbers => They could be as big as posible => Secund array will squared number of the first
// Will if all the squares of the fist array values are inside of the secund array =>  ? true: false

function comp(arr1,arr2){
    let sqrtVal = 0;
    for(let i = 0; i < arr2.length; i++){
        if(arr1.indexOf((Math.sqrt(arr2[i]))) !== -1) sqrtVal++
    }

    if(sqrtVal === arr2.length) return true
    else return false
}
